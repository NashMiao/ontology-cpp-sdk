...
{
    // Constants
    OP_0 = 0x00, // An empty array of bytes is pushed onto the stack. =This is not
                 // a no-op: an item is added to the stack.
    OP_FALSE = OP_0,
    OP_PUSHBYTES1 = 0x01, // 0x01-0x4B The next opcode bytes is data to be pushed
                          // onto the stack
    OP_PUSHBYTES75 = 0x4B,
    OP_PUSHDATA1 = 0x4C, // The next byte contains the number of bytes to be
                         // pushed onto the stack.
    OP_PUSHDATA2 = 0x4D, // The next two bytes contain the number of bytes to be
                         // pushed onto the stack.
    OP_PUSHDATA4 = 0x4E, // The next four bytes contain the number of bytes to be
                         // pushed onto the stack.
    OP_1NEGATE = 0x4F,   // The number -1 is pushed onto the stack.
    // OP_RESERVED=0x50, // Transaction is invalid unless occuring in an
    // unexecuted OP_IF branch
    ...
    OP_APPCALL = 0x67,
    OP_SYSCALL = 0x68,
    OP_VERIFY = 0x69,          // Marks transaction as invalid if top stack value is not
                               // true. True is removed, but false is not.
    OP_DUPFROMALTSTACK = 0x6A, // Marks transaction as invalid.

    // Stack
    OP_TOALTSTACK = 0x6B,   // Puts the input onto the top of the alt stack. Removes
    ...
    OP_2OVER = 0x70,        // Copies the pair of items two spaces back in the stack to
                            // the front.
    OP_2ROT =
        0x71,        // The fifth and sixth items back are moved to the top of the stack.
    ...
    OP_TUCK = 0x7D,  // The item at the top of the stack is copied and inserted
    ...
    OP_LEFT =
        0x80, // Keeps only characters left of the specified point in a string.
    OP_RIGHT =
        0x81,       // Keeps only characters right of the specified point in a string.
    ...
    // OP_RESERVED1=0x89, // Transaction is invalid unless occuring in an
    // unexecuted OP_IF branch
    // OP_RESERVED2=0x8A, // Transaction is invalid unless occuring in an
    // unexecuted OP_IF branch

    // Arithmetic
    // Note: Arithmetic inputs are limited to signed 32-bit integers, but may
    ...
    OP_NOT = 0x91,       // If the input is 0 or 1, it is flipped. Otherwise the output
                         // will be 0.
    ...
    // OP_NUMEQUALVERIFY=0x9D, // Same as OP_NUMEQUAL, but runs OP_VERIFY
    // afterward.
    ...
    OP_LESSTHANOREQUAL =
        0xA1, // Returns 1 if a is less than or equal to b, 0 otherwise.
    OP_GREATERTHANOREQUAL =
        0xA2,         // Returns 1 if a is greater than or equal to b, 0 otherwise.
    OP_MIN = 0xA3,    // Returns the smaller of a and b.
    OP_MAX = 0xA4,    // Returns the larger of a and b.
    OP_WITHIN = 0xA5, // Returns 1 if x is within the specified range
    ...
    OP_HASH160 = 0xA9,   // The input is hashed twice: first with SHA-256 and then
                         // with RIPEMD-160.
    OP_HASH256 = 0xAA,   // The input is hashed two times with SHA-256.
    // OP_CODESEPARATOR=0xAB, // All of the signature checking words will only
    // match signatures to the data after the most recently-executed
    // OP_CODESEPARATOR.
    OP_CHECKSIG = 0xAC, // The entire transaction's outputs, inputs, and script
                        // =from the most recently-executed OP_CODESEPARATOR to
                        // the end are hashed. The signature used by OP_CHECKSIG
                        // must be a valid signature for this hash and public key.
                        // If it is, 1 is returned, 0 otherwise.
    // OP_CHECKSIGVERIFY=0xAD, // Same as OP_CHECKSIG, but OP_VERIFY is executed
    // afterward.
    OP_CHECKMULTISIG =
        0xAE, // For each signature and public key pair, OP_CHECKSIG is executed.
              // If more public keys than signatures are listed, some key/sig
              // pairs can fail. All signatures need to match a public key. If all
              // signatures are valid, 1 is returned, 0 otherwise. Due to a bug,
              // one extra unused value is removed from the stack.
    // OP_CHECKMULTISIGVERIFY=0xAF, // Same as OP_CHECKMULTISIG, but OP_VERIFY is
    // executed afterward.
...
class ScriptOpMethod
{
  public:
    static unsigned char getByte(ScriptOp s_op)
    {
        return (unsigned char)s_op;
    }
};
#endif